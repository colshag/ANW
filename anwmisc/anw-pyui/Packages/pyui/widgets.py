...
class Label(Base):
    ...
    def __init__(self,text, color = None, font = None, shadow=0, align=0, type=0):
        Base.__init__(self)
        self.font = font
        self.shadow = shadow
        self.align = align
        ...
        if color:
            ...
            if type == 0:
                ...
                self.color = self.fgColor1
            elif type == 2:
                self.color = self.fgColor2
        self.setText(text)
    ...
    def draw(self, renderer):
        if self.show:
            getTheme().drawLabel(self.windowRect, self.text, color=self.color, font=self.font, shadow=self.shadow, align=self.align, type=self.type)
...
class Button(Base):
    ...
    def _pyuiMouseDown(self, event):
        ...
        self.getFocus()
        self.capture = 1
        getDesktop().getTheme().setWaitCursor()
        self.setDirty()
        return 1
...
class NumberEdit(Edit):
    ...
    def _pyuiKeyDown(self, event):
        ...
        # handle key presses - these are really handled by onChar
        if event.key < 45 or event.key == 47 or event.key > 57:
            return 0
        
        if event.key < 45 and len(self.text) != 0:
            return 0
    ...
    def getValue(self, returnFloat=1):
        ...
        try:
            if self.allowDecimels or returnFloat==1:
...
class ListBox(Base):
    """List Box has a scrollable list of selectable items.
       Added Multi-Select: Chris Lewis, Feb 10, 2006
    """
    canTab = 1    
    widgetLabel = "ListBox"               
    def __init__(self, onSelected = None, onDouble = None, w = 100, h = 100, enableMultiSelect = 0):
        Base.__init__(self)
        self.enableMultiSelect = enableMultiSelect # 1 -> enableMultiSelect
        self.dSelected = {} # key=item data, value= 1 -> selected, 0 -> unselected
        self.items = []
    ...
    def addItem(self, itemText, itemData, color = None):
        ...
        if self.enableMultiSelect == 1:
            self.dSelected[item.data] = 0
        self.setDirty()
        ...
        if self.show:
            ...
            for item in self.items:
                if i >= self.topItem and i < self.topItem + self.numVisible:
                    # if multi select use dSelected to decide if multi items can be highlighted
                    if self.enableMultiSelect == 1:
                        selected = self.dSelected[item.data]
                    else:
    ...
    def _pyuiLButtonDown(self, event):
        ...
        try:
            # left click will select or de-select an item
            if self.enableMultiSelect == 1 and len(self.items) > 0:
                item = self.items[self.selected]
                if self.dSelected[item.data] == 1:
                    self.dSelected[item.data] = 0
                else:
                    self.dSelected[item.data] = 1
        except IndexError:
            # if the user clicks on a blank list item this error occurs
            pass
    ...
    def getMultiSelectedItems(self):
        """Return the data values of all items currently selected as a list"""
        listSelected = []
        if self.enableMultiSelect == 1:
            for key, value in self.dSelected.iteritems():
                if value == 1:
                    listSelected.append(key)
        return listSelected
    ...
    def setSelectedItemByData(self, data):
        i=0
        for item in self.items:
            if item.data == data:
                self.selected = i
    ...
    def clearSelection(self):
        ...
        # deselect all items if multiselect enabled
        if self.enableMultiSelect == 1:
            for key, value in self.dSelected.iteritems:
                value = 0
