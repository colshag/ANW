...
class GridControl(gridlib.Grid, mixins.GridAutoEditMixin):
    ...
    def loadData(self, dictData, num):
        """Load data into Grid"""
        cellData = []
        keyList = anwp.func.funcs.sortDictValues(dictData)
        for obj in keyList:
        ...
        for list in cellData:
            ...
            if id <> 0 and id <> '':
                ...
                if dictObj.has_key(id):
                    ...
                    if dictName == 'empires':
                        newObj = anwp.aw.empire.Empire(gridValues)
                    elif dictName == 'systems':
                        newObj = anwp.aw.system.System(gridValues)
                    elif dictName == 'tradeRoutes':
                        newObj = anwp.aw.traderoute.TradeRoute(gridValues)
                    elif dictName == 'industrydata':
                        newObj = anwp.aw.industrydata.IndustryData(gridValues)
                    elif dictName == 'marketOrders':
                        newObj = anwp.aw.order.MarketOrder(gridValues)
                    elif dictName == 'marketStats':
                        newObj = anwp.aw.marketstat.MarketStat(gridValues)
                    elif dictName == 'componentdata':
                        newObj = anwp.war.componentdata.ComponentData(gridValues)
                    elif dictName == 'shiphulldata':
                        newObj = anwp.war.shiphulldata.ShipHullData(gridValues)
                    elif dictName == 'dronehulldata':
                        newObj = anwp.war.shiphulldata.DroneHullData(gridValues)
                    elif dictName == 'weapondata':
                        newObj = anwp.war.weapondata.WeaponData(gridValues)
                    elif dictName == 'captains':
                        newObj = anwp.war.captain.Captain(gridValues)
                    elif dictName == 'components':
                        newObj = anwp.war.component.Component(gridValues)
                    elif dictName == 'ships':
                        newObj = anwp.war.ship.Ship(gridValues)
                    elif dictName == 'regiments':
                        newObj = anwp.war.regiment.Regiment(gridValues)
                    elif dictName == 'ships':
                        newObj = anwp.war.ship.Ship(gridValues)
                    elif dictName == 'shipDesigns':
                        newObj = anwp.war.shipdesign.ShipDesign(gridValues)
                    elif dictName == 'weapons':
                        newObj = anwp.war.weapon.Weapon(gridValues)
                    # store obj in dict
                    dictObj[id] = newObj
            i += 1
...
class TestFrame(wx.Frame):
    def __init__(self, parent, log):
        ...
        self.grid = GridControl(self, log, anwp.func.root.Root({}), 4)
