...
class EmpirePanel(wx.Panel):
    def __init__(self, parent, id, log, galaxy):
        ...
        self.mainGrid = gridControl.GridControlParent(self, log, anwp.aw.empire.Empire({}), 1)
...
class SubNotebook(notebookControl.NotebookControl):
    def __init__(self, parent, id, log, galaxy, myID):
        ...
        # init tabs and grids
        d1 = ('techTree', anwp.aw.tech.Tech({}), 1)
        d2 = ('techOrders', anwp.aw.order.Order({}), 5)
        d3 = ('industryOrders', anwp.aw.order.IndustryOrder({}), 5)
        d4 = ('mailBox', anwp.aw.mail.Mail({}), 5)
        d5 = ('diplomacy', anwp.aw.diplomacy.Diplomacy({}), 1)
        
        self.MakeTabs(d1, d2, d3, d4, d5)
        
        # build these tabs with ability to populate Notebooks below them
        self.shipDesignsGrid = gridControl.GridControlShipDesign(self, log, anwp.war.shipdesign.ShipDesign({}), 5)
        self.AddPage(self.shipDesignsGrid, 'shipDesigns')
        self.droneDesignsGrid = gridControl.GridControlDroneDesign(self, log, anwp.war.dronedesign.DroneDesign({}), 5)
...
class TestFrame(wx.Frame):
    def __init__(self, parent, log):
        ...
        self.panel = EmpirePanel(self, -1, log, myGalaxy)
